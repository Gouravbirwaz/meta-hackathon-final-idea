...
### Results: The Learning Pattern

*The following plot demonstrates the training progress of KisanAgent using GRPO.*

![KisanAgent Training Dashboard](training_dashboard.png)

**Key Observations:**
1. **Early Exploration**: In the first 100 steps (Easy difficulty), the agent explores tool usage and irrigation basics.
2. **Strategy Convergence**: Between steps 150-350 (Medium), the agent learns to prioritize `pest_alert` and `mandi_price` before making high-stakes decisions.
3. **Closing the Gap**: The trained agent significantly outperforms the random baseline, achieving a net income consistently above ₹25,000, effectively closing the "data vacuum" gap for Harish.
...
- **Training**: Unsloth + TRL GRPO on Hugging Face Jobs (NVIDIA A10G-Large)
